    ...
    <div className="prose prose-slate max-w-none">
      <ReactMarkdown
        ...
        components={{
          ...
          // Table styles
          table: ({ node, ...props }) => (
            <div className="overflow-x-auto my-8 border border-forge-700 rounded-lg shadow-sm">
              <table className="min-w-full divide-y divide-forge-700" {...props} />
            </div>
          ),
          thead: ({ node, ...props }) => <thead className="bg-forge-800" {...props} />,
          tbody: ({ node, ...props }) => <tbody className="bg-white divide-y divide-forge-700" {...props} />, // Force white bg for table body to ensure readability vs dark mode forge-900 usually
          tr: ({ node, ...props }) => <tr className="hover:bg-forge-50 transition-colors" {...props} />,
          th: ({ node, ...props }) => (
            <th className="px-6 py-3 text-left text-xs font-medium text-forge-500 uppercase tracking-wider" {...props} />
          ),
          td: ({ node, ...props }) => (
            <td className="px-6 py-4 whitespace-nowrap text-sm text-forge-muted" {...props} />
          )
        }}
      >
        {content}
