...
export const generateDesignPrompts = async (prd: string, plan: string): Promise<{ stitch: string, opal: string }> => {
  ...
  const [stitchRes, opalRes] = await Promise.all([
    generateContentWithRetry(ai, 'gemini-1.5-flash-001', stitchPrompt, { temperature: 0.7 }),
    generateContentWithRetry(ai, 'gemini-1.5-flash-001', opalPrompt, { temperature: 0.7 })
  ]);
